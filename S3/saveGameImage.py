...
# CSV 파일 경로 설정
csv_directory = r'C:\ssafy3\rec\S09P31A104\S3'
...
with open(output_csv_file, 'w', newline='', encoding='utf-8') as csvfile:
    fieldnames = ['s3_image_url']  # S3 URL을 저장할 필드명
    ...
    with open(csv_file, 'r', encoding='utf-8-sig') as csvfile:
        ...
        for row in csvreader:
            ...
            image_name = image_key.split('.')[0]  # 파일 이름에서 확장자를 제외한 부분을 추출

            s3_image_key = f'{image_directory}{image_name}.jpg'  # S3에 저장할 경로+이미지 파일 이름
            ...
            if response.status_code == 200:
                ...
                # S3에 이미지 업로드
                image_file_path = os.path.join(image_path, image_key)
